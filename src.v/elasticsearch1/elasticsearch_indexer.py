...
PG_DB   = os.environ.get("POSTGRES_DB", "finance")
...
def fetch_enriched_records(pg_conn) -> list[dict]:
    """Query the enriched_stock_data VIEW from PostgreSQL."""
...
def main() -> None:
    ...
    for attempt in range(1, 16):
        try:
            ...
            logging.warning(f"PostgreSQL not ready ({exc}) — attempt {attempt}/15. Retrying in 5s …")
            time.sleep(5)
    ...
    if not records:
        logging.warning("No records found in PostgreSQL — nothing to index. "
                        "Make sure the Airflow DAG has run at least once.")
        return
